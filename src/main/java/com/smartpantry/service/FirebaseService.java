...
public class FirebaseService {
  ...
  private static final String PANTRY_COLLECTION = "pantry";
  ...
  public void saveCredentialPath(String path) throws IOException {
    ...
    try (var out = Files.newOutputStream(CONFIG_FILE)) {
      props.store(out, "SmartPantry config - path to your Firebase service account key");
    }
  }
  ...
  public void updateIngredient(Ingredient ingredient) throws ExecutionException, InterruptedException {
    if (ingredient.getId() == null) {
      throw new IllegalArgumentException("Ingredient has no id - did you mean addIngredient?");
    }
    ApiFuture<WriteResult> future = db.collection(PANTRY_COLLECTION)
  ...
  public void deleteIngredient(String id) throws ExecutionException, InterruptedException {
    ApiFuture<WriteResult> future = db.collection(PANTRY_COLLECTION).document(id).delete();
    future.get();
  }
}
