...
public class ShoppingListController {
    ...
    private void onRefresh() {
        ...
        Task<List<ShoppingItem>> task = new Task<>() {
            @Override protected List<ShoppingItem> call() throws Exception {
    ...
    private void onAdd() {
        ...
        if (name == null || name.isBlank()) { setStatus("Enter an item name", false); return; }
        double qty;
        try {
            qty = quantityField.getText().isBlank() ? 1.0 : Double.parseDouble(quantityField.getText());
        } catch (NumberFormatException ex) { setStatus("Quantity must be a number", false); return; }
        ...
        task.setOnSucceeded(e -> {
            nameField.clear(); quantityField.clear(); unitField.clear();
            onRefresh();
        });
    ...
    private void onToggleChecked() {
        ...
        if (selected == null || selected.getId() == null) { setStatus("Select an item first", false); return; }
    ...
    /** Moves the selected checked item into pantryItems then removes it from the shopping list. */
    @FXML
    ...
    private void onDelete() {
        ShoppingItem selected = listView.getSelectionModel().getSelectedItem();
        if (selected == null || selected.getId() == null) { setStatus("Select an item to delete", false); return; }
        Task<Void> task = new Task<>() {
            @Override protected Void call() throws Exception {
                firebaseService.deleteShoppingItem(selected.getId());
                return null;
    ...
    @FXML private void onNavRecipes()  { nav(Nav.Screen.RECIPES); }
    @FXML private void onNavShopping() { /* already here */ }
    @FXML private void onNavProfile()  { nav(Nav.Screen.PROFILE); }

    private void nav(Nav.Screen screen) {
        try { Nav.go((Stage) listView.getScene().getWindow(), screen); }
        catch (Exception ex) { setStatus("Navigation error: " + ex.getMessage(), false); }
    }
