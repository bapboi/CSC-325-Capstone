...
public class AddIngredientController {
    ...
    @FXML private Label photoStatusTitle;
    @FXML private Label photoStatusSubtitle;
    ...
    @FXML private ListView<String> pantryListView;
    ...
    private String formatForDisplay(Ingredient ingredient) {
        String category = (ingredient.getCategory() == null || ingredient.getCategory().isBlank())
                ? "Uncategorized" : ingredient.getCategory();
        return String.format("%s - %s %s (%s)",
                ingredient.getName(), ingredient.getQuantity(), ingredient.getUnit(), category);
    }

    /** Shared handler for both "Take Photo" and "Upload from Library" - both just open a file picker for now. */
    @FXML
    ...
    @FXML
    private void onOpenRecipes() {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/com/smartpantry/fxml/RecipeSuggestion.fxml"));
            Parent root = loader.load();
            Stage stage = (Stage) connectButton.getScene().getWindow();
            stage.setScene(new Scene(root, 420, 760));
        } catch (IOException e) {
            setStatus("Failed to open recipes: " + e.getMessage(), false);
        }
    }
